...
from agent import CoRagAgentWithPHS, RagPath
...
e5_ip = args.e5_ip
assert vllm_ip is not None, "VLLM IP address must be set"
assert e5_ip is not None, "E5 IP address must be set"
...
def main():
    ...
    ds: Dataset = load_dataset('corag/multihopqa', args.eval_task, split=args.eval_split, num_proc=32)
    ...
    predictions: List[List[str]] = ds['predictions']
